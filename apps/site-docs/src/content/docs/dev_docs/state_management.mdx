...
the frontend always demands the data to be in the latest schema no matter the stored schema version. so the backend need to run migration and provide frontend the required data
...
## Frontend
the fronend always talks in the latest schema and also speaks with a customized schema for frontend that doesn't mention local or synced states and schema version of any
